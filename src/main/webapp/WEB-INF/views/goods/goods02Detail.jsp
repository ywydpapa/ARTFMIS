...
function numberWitioutCommas(x) {
	...
	function fn_goodsUpdate01() {
		...
		.done(function(data) {
			if (data.code == 10001) {
				alert("저장 성공");
				var url = "${path}/goods/listGoods02/" + $("#selcode02").val();
				fn_Reload03(url);
			} else {
				alert("저장 실패");
			}
	...
	$(function() {
		$("#GOODS_NET_PRICE,#GOODS_SALE_UNIT").on('keyup', function() {
			var sum1 = parseInt($("#GOODS_NET_PRICE").val() || 0);
			var sum2 = parseInt($("#GOODS_SALE_UNIT").val() || 0);
			var sum = sum1 * sum2;
			$("#GOODS_SALE_PRICE").val(sum);
		});
	});
