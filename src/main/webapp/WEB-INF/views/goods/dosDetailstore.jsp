						...
						<tr>
							...
							<td class="align-middle"
								style="text-align: right; background-color: #E6F8E0"><button
									class="btn form-control btn-primary" onClick="fn_Dos3insert()">저장</button></td>
						</tr>
					</table>
				</div>
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<thead>
							...
							<tr style="text-align: center; background-color: #CED8F6">
								...
								<th style="text-align: center;">표준주문서등록여부(<a
							...
							<c:forEach var="row" items="${list}">
								<tr>
									...
									<td
										...
										max="100"
										value="<c:if test="${row.GOODS_INIT_BASIC eq 'N'}">0</c:if><c:if test="${row.GOODS_INIT_BASIC eq 'Y'}"><c:if test="${row.STORE_GOODS_INIT_QTY eq null }">1</c:if><c:if test="${row.STORE_GOODS_INIT_QTY ne null }">${row.STORE_GOODS_INIT_QTY}</c:if></c:if>"
									...
									<td
										...
										value="<c:if test="${row.GOODS_ADD_SALE eq 'N'}">0</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}"><c:if test="${row.STORE_GOODS_ADD_QTY eq null }">1</c:if><c:if test="${row.STORE_GOODS_ADD_QTY ne null }">${row.STORE_GOODS_ADD_QTY}</c:if></c:if>"
										min="0" max="100"
									...
									<th style="text-align: center; vertical-align: middle"
										...
										<c:if test="${row.STORE_GOODS_INIT_QTY ne null }">checked</c:if>></th>
								</tr>
							</c:forEach>
							...
							if (data.code == 10001) {
								...
								for (var i = 0; i < $Aarr.length; i++) {
									if ($($Chkarr[i]).is(":checked") == true) {
										...
										goodsData.STORE_GOODS_SALE_PRICE = Number($infoarr[i].innerText
												.replace(/[\D\s\._\-]+/g, ""));
