					...
					<table class="table table-bordered table-hover">
						...
						<tbody>
							<tr>
								...
								<td style="vertical-align:middle;text-align:center;" colspan="2"><input type="text" class="form-control" value="" id="GOODS_02TEXT" readonly></td>
								<td style="vertical-align:middle;text-align:center;" ><button class="btn btn-md btn-success" value="추가"	onClick="fn_setinsertmode()" style="margin-right:10px;">새항목 추가</button><button class="btn btn-md btn-primary"  onClick="fn_goodsUpdate01()">저장</button></td>
							</tr>
							<tr>
								<td style="vertical-align:middle;text-align:center;background-color:#E6F8E0">품명</td>
								<td style="vertical-align:middle;text-align:center"colspan="5"><input type="text" class="form-control"
							...
							<tr>
								...
								<td style="vertical-align:middle;text-align:center;"><input type="text" class="form-control curr CHKcal"
									style="text-align: right;" required name="GOODS_NET_PRICE"
									id="GOODS_NET_PRICE" onkeyup="numberWithCommas(this.value)" 
									value="<fmt:formatNumber value="${dto.GOODS_NET_PRICE}" pattern="#,###"/>" /></td>
							</tr>
							<tr>
								...
								<td colspan="3"><input  type="text" class="form-control curr"
...
function numberWitioutCommas(x) {
	...
	function fn_goodsUpdate01() {
		...
		.done(function(data) {
			if (data.code == 10001) {
				alert("저장 성공");
				var url = "${path}/goods/listGoods01/" + $("#selcode02").val();
				fn_Reload03(url);
			} else {
				alert("저장 실패");
			}
