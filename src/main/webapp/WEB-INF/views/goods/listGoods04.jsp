...
<div>
    <table class="table  table-bordered nowrap " id="goodslist">
        ...
        <tbody>
            <c:forEach var="row" items="${listgoods04}" varStatus="status">
            <tr>
              <td class="first" valign="middle">기타용품</td>
              <td class="first" valign="middle">${row.CAT_02_TITLE}</td>
              <td <c:if test="${status.first}">class="active"</c:if>><a href="javascript:fn_Reload04('${path}/goods/detail04/${row.GOODS_ID}')">${row.GOODS_TITLE}</a></td>
