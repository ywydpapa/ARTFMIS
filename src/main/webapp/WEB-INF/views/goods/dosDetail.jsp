					...
					<table class="table table-bordered table-hover">
						...
						<tbody>	
							...
							<tr style="vertical-align:middle">
								...
								<td <c:if test="${row.GOODS_ADD_SALE eq 'N'}">onClick="alert('초도주문 불가 품목 입니다.')" class="table-warning"</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}">class="table-warning"</c:if>  style = "text-align:right;vertical-align:middle" ><input class="from-control A <c:if test="${row.GOODS_INIT_BASIC eq 'N'}">table-warning</c:if><c:if test="${row.GOODS_INIT_BASIC eq 'Y'}">table-warning</c:if> " onchange="calculate()"
								...
								<td <c:if test="${row.GOODS_ADD_SALE eq 'N'}">onClick="alert('추가주문 불가 품목 입니다.')" class="table-warning"</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}">class="table-warning"</c:if> style = "text-align:right;"><input class="from-control C <c:if test="${row.GOODS_ADD_SALE eq 'N'}">table-warning</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}">table-warning</c:if>" onchange="calculate()"
								oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" style="border:none; text-align:right;" 
