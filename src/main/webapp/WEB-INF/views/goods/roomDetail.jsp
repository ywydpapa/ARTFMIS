...
<c:set var="path" value ="${pageContext.request.contextPath}"/>
					...
					<table  class="table table-sm bst02">
						...
						<tbody>
							<tr>
								<th scope="row"><input type="hidden" id="FROOM_ID" value ="${dtoRoom.FROOM_ID}">
									 시설구분
								</th>
								<td style="text-align:left;"><input type="text" readonly value="" id="typename" class="from-control form-control-sm"></td>
								<td class="Bset">호실타입</td>
								<td class="Bset">
									<select name="roomType2" id="roomType2" class="Bset form-control form-control-sm">
							...
							<button class="btn btn-md btn-success" value="추가" onClick="fn_setinsertmode()">새호실 추가</button>
								...
								<th scope="row">호실명</th>
								<td colspan="4">
									<input type="text" class="form-control form-control-sm" id="roomTitle" name="roomTitle" value="${dtoRoom.FROOM_TITLE}">
								</td>
								<tr>
								<th scope="row">일일사용료</th>
								<td>
									<input type="text" style = "text-align:right;" class="form-control form-control-sm CHK" name="priceDay" id="priceDay" value="<fmt:formatNumber value="${dtoRoom.FROOM_DAY_PRICE}" pattern="#,###"/>">
								</td>
								<th scope="row">시간당사용료</th>
								<td colspan="2">
									<input type="text" id="priceHour" name="priceHour" style = "text-align:right;" class="form-control form-control-sm CHK" name="priceHour" value="<fmt:formatNumber value="${dtoRoom.FROOM_TIME_PRICE}" pattern="#,###"/>">
								</td>
            					</tr>
								<tr class="Bset">
								<th scope="row">면적(m<sup>2</sup>)</th>
								<td>
									<input type="number" class="Bset form-control form-control-sm" id="roomSize" name="roomSize" value="${dtoRoom.FROOM_AREA}">
								</td>
								<th scope="row">평수</th>
								<td  colspan="2">
								<input type="number" style = "text-align:right;" class="Bset form-control form-control-sm" name="pyungSize" id="pyungSize" value="${dtoRoom.FROOM_AREA_KOR}">
								</td>
								</tr>
								<tr class="Bset">
								<th scope="row">수용인원수</th>
								<td>
									<input name="roomCap" id="roomCap" style = "text-align:right;" class="Bset form-control form-control-sm" type="number" value="${dtoRoom.FROOM_MAX_PERS}">
								</td>
								<th scope="row">컴퓨터이름</th>
								<td  colspan="2">
									<input type="text" class="Bset form-control form-control-sm" id="pcTitle" name="pcTitle" value="${dtoRoom.FROOM_PC}">
								</td>
							</tr>
							<tr class="Bset">
								<th scope="row">임시호실명</th>
								<td>
									<input type="text" class="Bset form-control form-control-sm" id="temproomTitle" name="temproomTitle" value="${dtoRoom.FROOM_ALIS}">
								</td>
								<th scope="row">임시호실사용여부</th>
								<td  colspan="2">
									<select name="imsiYn" id="imsiYn" class="Bset form-control form-control-sm">
							...
							<tr>
								...
								<td>
									<input type="number" style = "text-align:right;" class="form-control form-control-sm" id="orderNo" name="orderNo" value="${dtoRoom.FROOM_ORDNO}">
								</td>
							...
							<tr>
								...
								<td>
									<form id="uploadForm" enctype="multipart/form-data" method="post">
										<input type="hidden" id="uploadServerImageName" value="${dtoRoom.FROOM_IMAGE}">
							...
							<tr>
								<th scope="row">내용</th>
								<td colspan="4">
									<textarea name="roomDesc" id="roomDesc" rows="8" class="form-control">${dtoRoom.FROOM_REMARK}</textarea>
								</td>
							</tr>
						</tbody>
					</table>
