	...
	<div class="row">
		<div class="col-sm-12">
			<div class="card-block table-border-style">
				...
				<td class="align-middle" style="text-align:right;background-color:#E6F8E0"><button class="btn form-control btn-primary" onClick="fn_Dos2insert()">저장</button></td>
				</tr>
				</table>
				</div>
				<div class="table-responsive">
					<table id = "DOS01" class="table table-bordered table-hover">
						...
						<tbody>	
							...
							<tr>
								...
								type="text"  min="0" max="100" value="<c:if test="${row.GOODS_INIT_BASIC eq 'N'}">0</c:if><c:if test="${row.GOODS_INIT_BASIC eq 'Y'}"><c:if test="${row.MESSR_GOODS_INIT_QTY eq null }">1</c:if><c:if test="${row.MESSR_GOODS_INIT_QTY ne null }">${row.MESSR_GOODS_INIT_QTY}</c:if></c:if>"
								<c:if test="${row.GOODS_INIT_BASIC eq 'N'}">disabled</c:if> required></td>
								<td class="B" style = "text-align:right;vertical-align:middle"></td>
								<td <c:if test="${row.GOODS_ADD_SALE eq 'N'}">onClick="alert('추가주문 불가 품목 입니다.')" class="table-warning"</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}">class="table-warning"</c:if>  style = "text-align:right;vertical-align:middle"><input class="from-control C table-warning" oninput="calculate()" style="border:none; text-align:right;" 
								type="text" value ="<c:if test="${row.GOODS_ADD_SALE eq 'N'}">0</c:if><c:if test="${row.GOODS_ADD_SALE eq 'Y'}"><c:if test="${row.MESSR_GOODS_ADD_QTY eq null }">1</c:if><c:if test="${row.MESSR_GOODS_ADD_QTY ne null }">${row.MESSR_GOODS_ADD_QTY}</c:if></c:if>" min="0" max="100" 
	...
	function fn_Dos2insert() {
		...
		.done(function(data) {
			if (data.code == 10001) {
				...
				goodsData.MESSR_GOODS_SALE_PRICE = Number($infoarr[i].innerText.replace(/[\D\s\._\-]+/g, ""));
