...
<td>
<select class="form-control" id="selcode01">
...
<td>중분류</td>
<td>
<select class="form-control" id="selcode02">
	<c:forEach var="row" items = "${listsel02}" varStatus="status">
		<c:if test="${status.index eq 0 }"><option value="${row.CAT_ID}" selected>${row.CAT_TITLE}</option></c:if>
		<c:if test="${status.index ne 0 }"><option value="${row.CAT_ID}">${row.CAT_TITLE}</option></c:if>
		<%-- <option value="${row.CAT_ID}">${row.CAT_TITLE}</option> --%>
	</c:forEach>
</select>
</td>
...
$("#selcode02").change(function(){
	var url ="${path}/goods/listGoods0"+$("#selcode01").val()+"/"+$("#selcode02").val();
	setclear();
	tableListLoad(url);
});
...
function tableDetailLoad(){
	if ($("#goodslist > tbody > tr > td > a").length > 0){
		var tempArr = $("#goodslist > tbody > tr > td > a").eq(0).attr('href').split('/');
		var num = tempArr[tempArr.length - 1].replace(')','').replace('"','').replace('\'','');
		var url ="${path}/goods/detail0"+$("#selcode01").val()+"/"+num;
		$.ajax({
			type: "GET",
			url : url,
			dataType : "html",
