...
<div>
  <table class="table  table-bordered nowrap " id="goodslist">
      ...
      <tbody>
          <c:forEach var="row" items="${listgoods03}" varStatus="status">
            <tr>
              <td class="first" valign="middle">매점용품</td>
              <td class="first" valign="middle">${row.CAT_02_TITLE}</td>
              <td <c:if test="${status.first}">class="active"</c:if>><a href="javascript:fn_Reload04('${path}/goods/detail03/${row.GOODS_ID}')">${row.GOODS_TITLE}</a></td>
